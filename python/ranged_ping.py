...
def ping_ip(ip, timeout):
    try:
        output = subprocess.check_output(["ping", "-c", "1", str(ip)], timeout=timeout, universal_newlines=True)
...
def find_accessible_ip(start_ip=DEFAULT_START_IP, end_ip=DEFAULT_END_IP, timeout=DEFAULT_TIMEOUT):
    accessible_count = 0
    inaccessible_count = 0
    total_time = 0
    average_time = 0
    
    total_ips_in_range = int(ipaddress.IPv4Address(end_ip)) - int(ipaddress.IPv4Address(start_ip)) + 1
