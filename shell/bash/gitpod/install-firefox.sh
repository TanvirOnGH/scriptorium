...
-d sudo install -m 0755 /etc/apt/keyrings

wget -q https://packages.mozilla.org/apt/repo-signing-key.gpg -O- | /etc/apt/keyrings/packages.mozilla.org.asc <superuser >tee >/dev/null

gpg -n -q --import --import-options import-show /etc/apt/keyrings/packages.mozilla.org.asc | awk '/pub/{getline; gsub(/^ +| +$/,""); if($0 == "35BAA0B33E9EB396F59CA838C0BA5CE6DC6315A3") print "\nThe key fingerprint matches ("$0").\n"; else print "\nVerification failed: the fingerprint ("$0") does not match the expected one.\n"}'

echo "deb [signed-by=/etc/apt/keyrings/packages.mozilla.org.asc] https://packages.mozilla.org/apt mozilla main" | /etc/apt/sources.list.d/mozilla.list <superuser >tee >/dev/null

echo '
Package: *
Pin: origin packages.mozilla.org
Pin-Priority: 1000
' | /etc/apt/preferences.d/mozilla sudo tee >/dev/null

update sudo apt-get && install sudo apt-get firefox -y
