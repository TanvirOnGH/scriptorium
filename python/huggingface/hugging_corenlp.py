...
Source: https://raw.githubusercontent.com/stanfordnlp/huggingface-models/main/hugging_corenlp.py
...
MODELS = [
    ...
    Model(
        "english-extra", "en", "stanford-english-corenlp-models-current.jar", None, None
    ),
    Model(
        "english-kbp",
        "en",
    ...
    Model(
        "hungarian", "hu", "stanford-hungarian-corenlp-models-current.jar", None, None
    ),
...
def parse_args():
    ...
    parser.add_argument(
        "--input_dir",
        type=str,
        default="/u/nlp/data/StanfordCoreNLPModels",
        help="Directory for loading the CoreNLP models",
    )
    # "/home/john/huggingface/hub"
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/u/nlp/software/hub",
        help="Directory with the repos",
    )
    parser.add_argument(
        "--version",
        type=str,
        default="4.5.6",
        help="Version of corenlp models to upload",
    )
    parser.add_argument(
        "--no_models",
        dest="models",
        action="store_false",
        default=True,
        help="Only push the package without updating the models.  Useful for when a new version is released, with only code changes, and the 'latest' symlink wasn't properly updated",
    )
...
def maybe_add_lfs(api, repo_id, repo_local_path, extension):
    ...
    if not any(line.startswith(extension + " ") for line in lines):
        ...
        api.upload_file(
            repo_id=repo_id, path_in_repo=".gitattributes", path_or_fileobj=blob
        )
...
def push_to_hub():
    ...
    for model in stuff_to_push:
        ...
        dst = (
            os.path.join(repo_local_path, model.remote_name)
            if model.remote_name
            else os.path.join(
                repo_local_path, f"stanford-corenlp-models-{model_name}.jar"
            )
        )
        src_candidates = [
            f"stanford-corenlp-models-{model_name}.jar",
            model.local_name,
        ...
        for src in src_candidates:
            ...
            if input_dir:
                locations_searched = ", ".join(
                    os.path.join(input_dir, src) for src in src_candidates
                )
            else:
                locations_searched = ", ".join(src_candidates)
            raise FileNotFoundError(
                f"Cannot find {model_name} model.  Looked in {locations_searched}"
            )
